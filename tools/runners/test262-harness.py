...
YAML_INCLUDES_LEGACY_RE = re.compile(r"includes:\s+(( *- (.*)\s+)*)", re.MULTILINE)
YAML_FLAGS_RE = re.compile(r"flags:\s+\[(?P<flags>.+)\]")
YAML_NEGATIVE_RE = re.compile(r"negative:.*(((phase:\s+(?P<phase>\w+))|(type:\s+(?P<type>\w+))).*){2}", re.DOTALL)
...
class TestCase(object):
    ...
    def parse_test_record(self):
        ...
        if match:
            ...
            match = YAML_INCLUDES_LEGACY_RE.search(header)
            self.test_record["includes"] = [inc.strip() for inc in match.groups()[0].split("-") if inc] if match else []

        match = YAML_FLAGS_RE.search(header)
        if match:
            for flag in match.group("flags").split(","):
                if flag.strip():
                    self.test_record[flag.strip()] = ""

        match = YAML_NEGATIVE_RE.search(header)
        if match and match.group("phase") and match.group("type"):
